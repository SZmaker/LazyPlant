...
int GAgent_Local_ctl(char *buf)
{
    ...
    led_g = buf[4];
    led_b = buf[5];
	GAgent_Printf(GAGENT_INFO,"Ryan Local_ctl %d %d %d %d %d %d",buf[0], buf[1], buf[2], buf[3], buf[4], buf[5]);
	...
	if(sub_cmd == 0x01){
		...
		if((cmd_tag & 0x01) == 0x01)
		{
			...
			if((cmd_byte & 0x01) == 0x01)
			{
				...
				system("/home/root/set_DIG.sh -o 13 -v low");		
				...
				rx_tx_buf[4] = 6;
				rx_tx_buf[5] = 55;
				mraa_i2c_write(i2c, rx_tx_buf, 6);
		...
		if((cmd_tag & 0x02) == 0x02)
		{
			...
			if(tmp_cmd_buf == 0x00)
			{
				...
				system("python /home/root/Rotate0.py");	
			...
			{		
				...
				system("python /home/root/Rotate90.py");	
				///system("/home/root/set_DIG.sh -o 9 -v low");					
			}
			else if(tmp_cmd_buf == 0x02)
			{
				...
				system("python /home/root/Rotate180.py");	
				///system("/home/root/set_DIG.sh -o 10 -v low");					
			}
			else if(tmp_cmd_buf == 0x03)
			{
				cmd_byte = (cmd_byte | 0x06);
				
				///system("/home/root/set_DIG.sh -o 11 -v low");					
			}
		}
		...
		if((cmd_tag & 0x04) == 0x04)
		{
			if(tmp_cmd_buf == 0x00){
				...
				GAgent_Printf(GAGENT_INFO,"led_r %d",led_r);
				if(led_r > 128)
				{
					mraa_i2c_context i2c;
					char rx_tx_buf[16];
	    		i2c = mraa_i2c_init(0);
					...
					system("/home/root/set_DIG.sh -o 5 -v low");	
				}
				else
				{
					mraa_i2c_context i2c;
		...
		if((cmd_tag & 0x08) == 0x08)
		{
			if(tmp_cmd_buf == 0x00){
				...
				GAgent_Printf(GAGENT_INFO,"led_g %d %d",led_g);
				if(led_g > 128)
				{
					mraa_i2c_context i2c;
					char rx_tx_buf[16];
					...
					system("/home/root/set_DIG.sh -o 6 -v low");	
				}
				else
				{
					mraa_i2c_context i2c;
		...
		if((cmd_tag & 0x10) == 0x10)
		{
			...
			GAgent_Printf(GAGENT_INFO,"led_b %d",led_b);
			if(led_b > 128)
			{
					mraa_i2c_context i2c;
					char rx_tx_buf[16];
					...
					system("/home/root/set_DIG.sh -o 7 -v low");	
			}
			else
			{
		...
		if((cmd_tag & 0x20) == 0x20)
		{
		    ...
		    GAgent_Printf(GAGENT_INFO,"motor_speed %d",motor_speed);
		    if(motor_speed > 4)
		    {
					mraa_i2c_context i2c;
					char rx_tx_buf[16];
		    ...
		    else
		    {
				  ...
				  ///system("python /home/root/tclient.py");	

				  if(g_Camera_Ready==1)
				  {
      		  	system("/home/root/wxupload -f plant.jpg");	
      		  	system("/home/root/fbupload -a /home/root/rl98.token -f plant.jpg -m \"LazyPlant test\"");
      		  	g_Camera_Ready = 0;
      		 }
		    }				    
		}
